...
def residual_block(x, cn, scope_name):
	with tf.variable_scope(scope_name):
		...
		b1 = bias_variable([cn])
		x1 = bn(relu(conv2d(x, w1) + b1))
		w2 = weight_variable([3, 3, cn, cn])
		b2 = bias_variable([cn])
		x2 = bn(conv2d(x1, w2) + b2)
...
def model(x):
	...
	x4 = residual_block(x3, 128, 'res1')
	x5 = residual_block(x4, 128, 'res2')
	x6 = residual_block(x5, 128, 'res3')
	x7 = residual_block(x6, 128, 'res4')
	x8 = residual_block(x7, 128, 'res5')

	# pad_x8 = tf.pad(x8, [[0 ,0], [1, 1], [1, 1], [0, 0]], 'CONSTANT')
	x9 =  bn(relu(conv2d_transpose(x8, 64, kernel_size=(4, 4), stride=(2, 2), padding='SAME', activation_fn=None)))
	# pad_x9 = tf.pad(x9, [[0 ,0], [1, 1], [1, 1], [0, 0]], 'CONSTANT')
	x10 =  bn(relu(conv2d_transpose(x9, 32, kernel_size=(4, 4), stride=(2, 2), padding='SAME', activation_fn=None)))
	
	w4 = weight_variable([9, 9, 32, 1])
	b4 = bias_variable([1])
	x11 = relu(conv2d(x10, w4))

	return x11, x10, x9, x3, x2, x1

def main(argv = None):
	#f_name = '../data/rectangle_m.npy'
	#save_prefix = '../result/trained_models_rectangle_multiple_resnet/'
	f_name = '../data/all_data_m.npy'
	save_prefix = '../result/trained_models_multiple_resnet/'
